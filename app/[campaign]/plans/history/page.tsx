...
export default function PlansHistoryPage() {
  ...
  const [dayDetail, setDayDetail] = useState<PlanNewHistoryEntry | null>(null);
  ...
  async function toggleDate(date: string) {
    ...
    try {
      ...
      setDayDetail(data.entries[0] || null);
    } catch {
      // silent
    } finally {
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  <div className="flex gap-2 text-xs">
                    <span className="text-green-700">{entry.posted}</span>
                    {entry.failed > 0 && <span className="text-red-600">{entry.failed}</span>}
                    <span className="text-muted-foreground">{entry.resting}r</span>
                  </div>
                ...
                <div className="px-3 pb-3">
                  {detailLoading ? (
                    ...
                    <div className="space-y-1 rounded-lg border p-3">
                      {Object.entries(dayDetail.groups).map(([name, stats]) => (
                        <div key={name} className="flex items-center justify-between text-sm py-1">
                          ...
                          <div className="flex gap-2 text-xs">
                            ...
                            <span className="text-muted-foreground">{stats.resting} rest</span>
                          </div>
                        </div>
                      ))}
                    </div>
            ...
            <div key={i} className="p-3 rounded-lg border bg-background border-border">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <Badge variant="outline" className="text-xs">{a.group}</Badge>
                  {a.region && <Badge variant="outline" className="text-xs">{a.region}</Badge>}
                </div>
            ...
            <div key={i} className="p-3 rounded-lg border bg-background border-border">
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2 text-xs text-muted-foreground">
                  <span>{entry.generation_count} generations</span>
                  <span>{entry.full_cycle_count} cycles</span>
                </div>
              </div>
